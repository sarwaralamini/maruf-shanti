<?php
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		...
		if(isset($cf_name)){
			...
			$message .= 'FirstName: ' . $cf_name . "\r\n";

		}

		if(isset($cf_lname)){

			$cf_lname = htmlspecialchars(filter_var($cf_name, FILTER_SANITIZE_STRING), ENT_QUOTES | ENT_HTML5);

			$message .= 'LastName: ' . $cf_lname . "\r\n";
		...
		if(isset($cf_country)){
			...
			$cf_country = htmlspecialchars(filter_var($cf_country, FILTER_SANITIZE_STRING), ENT_QUOTES | ENT_HTML5);

			$message .= 'Country: ' . $cf_country . "\r\n";
